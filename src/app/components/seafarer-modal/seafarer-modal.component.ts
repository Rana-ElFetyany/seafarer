  ...
  onSave() {
    if (this.seafarerForm.valid) {
      ...
      const payload = {
        entity: {
          // ✅ Id fix
          Id: this.isEditMode && this.currentId ? this.currentId : 0,

          // ✅ EmpId from employee object
          EmpId: formValue.EmployeeName?.Value ?? 0,
      ...
      request$.subscribe({
        next: (response) => {
          ...
          console.log('Saved ID:', this.currentId);
        },
